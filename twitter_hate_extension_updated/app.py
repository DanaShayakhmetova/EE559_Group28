...
# fallback functions for implied statement 
import nltk
...
def load_all_models():
    ...
    # RASCH
    # rasch_model_identifier = "gpt2_small"
    rasch_model_identifier = "gpt2-small-regression-finetuned-fixed-smaller-train"
    try:
        print(f"Loading Rasch tokenizer from: {rasch_model_identifier}")
        # extending torch.load to force CPU loading temporarily
        original_torch_load = torch.load
        torch.load = lambda *args, **kwargs: original_torch_load(*args, map_location='cpu', **kwargs)
        tokenizer, config, model = GPT2Regression.load_model(rasch_model_identifier)
        torch.load = original_torch_load
        rasch_tokenizer_global = tokenizer
        if rasch_tokenizer_global.pad_token is None:
            rasch_tokenizer_global.pad_token = rasch_tokenizer_global.eos_token
        rasch_model_global = model.to(device_global)
    ...
    # implicit_model_path = "../stats/gpt_combined_tokenized/gpt2_implicit_hate_combined/final_model"
    implicit_model_path="gpt-v4-small/gpt2_implicit_hate_combined_small/final_model"
    try:
...
def fallback_extract_target_group(raw: str) -> str:
    # COMMON_GROUPS = {
    #     "white people", "minorities", "immigrants", "black people", "non-white people",
    #     "jews", "muslims", "christians", "blacks", "asians", "lgbtq",
    #     "trans people", "women", "men", "whites", "white women", "black women"
    # }
    ...
    try:
        ...
        normalized_candidates = [c.lower() for c in parts]
        filtered = [c for c in normalized_candidates if 1 <= len(c.split()) <= 2]
        print(f"\nCandidates extracted for target group: {filtered}")


        # for idx, candidate in enumerate(normalized_candidates):
        #     for group in COMMON_GROUPS:
        #         if group in candidate:
        #             # Return the original candidate with proper capitalization
        #             return candidates[idx].capitalize()
        ...
        print(f"Fallback target group extraction error: {e}")
...
def fallback_extract_implied_statement(raw: str) -> str:
    try:
        segments = re.split(r'\s{2,}', raw.strip())
        segments = segments[3:]  # skip first 3
        print(f"\nSegments {segments}")

        # clean_sents = []
        for sent in segments:
            sent = sent.strip()
            if 3 <= len(sent.split()) <= 20 and re.search(
                r"\b(are|is|should|hate|must|attack|invade|target|destroy|expel|don’t|don't|do not)\b",
                sent, re.IGNORECASE
            ):
        ...
        # if clean_sents:
        #     ranked = Counter(clean_sents)
        #     return ranked.most_common(1)[0][0]

        return "an unspecified harmful implication"

    except Exception as e:
        print(f"Fallback implied statement extraction error: {e}")
        return "an unspecified harmful implication"
        ...
        if classification != "Non-Hate":
            ...
            if classification == "Implicit Hate":
                ...
                if target_group_match: target_group = target_group_match.group(1).strip()
                    ...
                    print(f"Warning: Could not parse 'Target Group' from: {raw_explanation} \n Using callback function.")
                    target_group = fallback_extract_target_group(raw_explanation)
                ...
                if implied_statement_match:
                    ...
                    print(f"Warning: Could not parse 'Implied Statement' from: {raw_explanation} \n Using fallback function.")
                    implied_statement = fallback_extract_implied_statement(raw_explanation)

                explanation_display = f"This tweet seems to be implicitly hateful because it is targeting {target_group.lower()} by saying {implied_statement.lower()}."
